...
class BlogCommentDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->addColumn('action', function($query){
            if($query->status === 1){
                $edit = "<a href='".route('admin.blogs.comments.update',$query->id)."' class='btn btn-primary'><i class='fas fa-eye'></i></a>";
            }else{                
                $edit = "<a href='".route('admin.blogs.comments.update',$query->id)."' class='btn btn-warning'><i class='fas fa-eye-slash'></i></a>";
            }

            $delete = "<a href='".route('admin.blogs.comments.destroy',$query->id)."' class='delete-item btn btn-danger ml-2'><i class='fa fa-trash'></i></a></a>";
            return $edit.$delete;
        ...
        ->rawColumns(['blog','user_name','date','action','status'])
            ->setRowId('id');
    }
